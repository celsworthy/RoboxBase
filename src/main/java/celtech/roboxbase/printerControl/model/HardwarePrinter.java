...
import celtech.roboxbase.comms.tx.*;
...
import celtech.roboxbase.printerControl.*;
...
import celtech.roboxbase.printerControl.model.statetransitions.calibration.*;
...
import celtech.roboxbase.utils.*;
...
import static celtech.roboxbase.printerControl.comms.commands.GCodeConstants.getEjectFilamentCommand;
...
//            commandInterface.writeToPrinter(readPacket);
    ...
    public void printProject(PrintableProject printableProject, Optional<GCodeGeneratorResult> potentialGCodeGenResult, boolean safetyFeaturesRequired) throws PrinterException {
        ...
        if (usedExtruders.get(0) && !usedExtruders.get(1)) {
            ...
            if (extruders.get(1).filamentLoadedProperty().get()
                ...
                ejectFilament(1, filament1.getNozzleTemperature() - 10, null);
            }
            ...
            if (extruders.get(0).filamentLoadedProperty().get()
                ...
                BaseLookup.getSystemNotificationHandler().showInformationNotification("", BaseLookup.i18n("notification.ejectingNotRequiredFilament"));
                ejectFilament(0, filament0.getNozzleTemperature() - 10, null);
            }
        } else {
    ...
    public void ejectFilament(int extruderNumber, int temperature, TaskResponder responder) throws PrinterException {
        ...
        new Thread(() ->
        {
            boolean success = doEjectFilamentActivity(extruderNumber, temperature, cancellable);
    ...
    private boolean doEjectFilamentActivity(int extruderNumber, int temperature, Cancellable cancellable) {
        boolean success = false;
        try {
            transmitDirectGCode(getEjectFilamentCommand(extruders.get(extruderNumber).getExtruderAxisLetter(), temperature), false);
            PrinterUtils.waitOnBusy(this, cancellable);
            success = true;
        } catch (RoboxCommsException ex) {
